 ...
 * @description: 近12个月贷记卡审批查询次数
 * @author: liuyafei
 * @create: 2019/10/23 19:24
 */
public class SecdQueryForCardLast12monContSum implements Function<JSONObject, Object>, RhParamKey {

    static String resultKey="secd_QueryForCardLast12monContSum";
     ...
     * =【3-信用卡审批】 且查询日期（QueryDate）距离报告头（Crt2_P_Identification）—》报告日期（ReportCreateTime）<=12个月的查询次数
    ...
    public Object apply(JSONObject jsonObject) {
        ...
        for(int i=0;i<queryrecord.size();i++){
            ...
            if(!StringUtils.isEmpty(jsonObject1.getString("queryDate"))){
                ...
                if("03".equals(jsonObject1.getString("inquiryCause"))&&getDaySub(queryDate,reportCreateTime)/30.0<=12){
