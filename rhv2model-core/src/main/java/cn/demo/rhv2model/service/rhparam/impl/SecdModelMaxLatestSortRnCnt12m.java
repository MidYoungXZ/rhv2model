 ...
 * @description: 近12个月最高逾期期数
 * @author: liuyafei
 * @create: 2020/01/06 14:54
 */
public class SecdModelMaxLatestSortRnCnt12m implements Function<JSONObject,Object>, RhParamKey{
    static String resultKey="secd_model_max_latest_sort_rn_cnt_12m";
    ...
    public Object apply(JSONObject jsonObject) {
        ...
        for (int i = 0; i < thelatestperformance.size(); i++) {
            ...
            if(set.contains(repayment.getString("accountNumber"))&&month<=12){
        ...
        for(String key : latestnstate.keySet()){
            if(num<latestnstate.get(key)){
                num=latestnstate.get(key);
            }
        }
        return num;
    }
