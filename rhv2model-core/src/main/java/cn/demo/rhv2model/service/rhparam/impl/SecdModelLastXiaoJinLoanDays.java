 ...
 * CreateDate: 2020/1/3
 */
public class SecdModelLastXiaoJinLoanDays implements Function<JSONObject, Object> , RhParamKey {

    public static String resultKey = "secd_model_last_xiaojin_loan_days";
    ...
    public Set checkAccountType(JSONObject jsonObject, String[] accountType) {
        ...
        for (int i = 0; i < essentialinformation.size(); i++) {
            ...
            if (Arrays.asList(accountType).contains(account.getString("accountType"))&&
                    "24".equals(account.getString("businessInstitutions"))){
    ...
    public Object apply(JSONObject jsonObject) {
        ...
        String max = (String) Collections.max(set);
        return getDaySub(max,reportCreateTime)==0?"-999":String.valueOf(getDaySub(max,reportCreateTime));
    }
