 ...
 * @description: 近3个月贷记卡审批查询次数
 * @author: liuyafei
 * @create: 2019/10/23 19:24
 */
public class SecdQueryForCardLast3monCount implements Function<JSONObject, Object>, RhParamKey {

    static String resultKey="secd_QueryForCardLast3monCount";
    ...
    /**
     * 1、查询记录（QueryRecord）—》 查询记录信息（Crt2_P_QueryRecord） —》查询原因（InquiryCause） =【3-信用卡审批】 且查询日期（QueryDate）距离报告头（Crt2_P_Identification）—》报告日期（ReportCreateTime）<=3个月的查询次数
     * 2、时间统计口径（报告查询日期-查询日期）/30<=3
    ...
    public Object apply(JSONObject jsonObject) {
        ...
        for(int i=0;i<queryrecord.size();i++){
            ...
            if(!StringUtils.isEmpty(jsonObject1.getString("queryDate"))){
                ...
                if("03".equals(jsonObject1.getString("inquiryCause"))&&getDaySub(queryDate,reportCreateTime)/30.0<=3){
