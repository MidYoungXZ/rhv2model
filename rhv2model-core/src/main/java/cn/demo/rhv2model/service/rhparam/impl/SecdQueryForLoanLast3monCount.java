 ...
 * @description: 近3个月贷款审批查询次数
 * @author: liuyafei
 * @create: 2019/10/23 19:24
 */
public class SecdQueryForLoanLast3monCount implements Function<JSONObject, Object>, RhParamKey {

    static String resultKey="secd_QueryForLoanLast3monCount";
    ...
    /**
     * 1、查询记录（QueryRecord）—》 查询记录信息（Crt2_P_QueryRecord） —》查询原因（InquiryCause） =【2-贷款审批】且查询日期（QueryDate）距离报告头（crt2_p_identification）—》报告日期（ReportCreateTime）<=3个月的查询次数
    ...
    public Object apply(JSONObject jsonObject) {
        ...
        for(int i=0;i<queryrecord.size();i++){
            ...
            if(!StringUtils.isEmpty(jsonObject1.getString("queryDate"))){
                ...
                if("02".equals(jsonObject1.getString("inquiryCause"))&&getDaySub(queryDate,reportCreateTime)/30.0<=3){
