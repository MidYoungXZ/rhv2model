...
public class NanjingReasonScore implements CodeScoreRule<NanjingReasonScore.ReasonRuleParam> {
    ...
    public Map<String, Object> getCodeRuleScore(ReasonRuleParam codeRuleParam) {
        ...
        if (codeRuleParam.getCardBaddebtCount() > 0||
                codeRuleParam.getLoanBaddebtCount() > 0||
                codeRuleParam.getBadLoanCount() > 0||
                codeRuleParam.getStandardCardBaddebtCount() > 0||
                codeRuleParam.getJudgementCount() > 0||
                codeRuleParam.getFellbackCount() > 0) {
            reasonScore.append(",A");
        }
        ...
        if (codeRuleParam.getLoanOverdueNumLast24MonMax() > 7||codeRuleParam.getCardOverdueNumLast24MonMax() > 7||codeRuleParam.getStandardCardOverdueNumLast24MonMax() > 7) {
            reasonScore.append(",B");
        }



        if (codeRuleParam.getCardOverdueNumLast12MonMax() > 2||
                codeRuleParam.getCardOverdueNumLast6MonMax() > 1||
                codeRuleParam.getCardOverdueLast12MonContSum() > 3||
                codeRuleParam.getCardOverdueLast6MonContSum() > 2||
                codeRuleParam.getSecdCustomerLoanOverdueLast6MonContSum() > 0||
                codeRuleParam.getSecdCustomerLoanOverdueLast12MonContSum() > 1||
                codeRuleParam.getSecdLoanOverdueNumLast12MonMax() > 1

        ) {
            reasonScore.append(",C");
        }
        ...
        if (codeRuleParam.getCardOverdueAmtSum() > 0||
                codeRuleParam.getStandardCardOverdueAmt() > 500||
                codeRuleParam.getLoanOverdueAmtSum() > 0
        ) {
            reasonScore.append(",D");
        }



        if (codeRuleParam.getQueryForloanCardLast3monContSum() > 12||
                codeRuleParam.getFarmingLoanAmtSum() > 300000||
                codeRuleParam.getGuaranteeAmtSum() > 500000) {
            reasonScore.append(",E");
        }
        ...
        res.put("rigidRefuse", reasonScore.length() > 0 ? reasonScore.toString().substring(1) : "9999");
