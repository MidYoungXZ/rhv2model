 ...
 * @description: 流通贷记卡的总授信额度
 * @author: liuyafei
 * @create: 2019/10/23 17:35
 */
public class SecdNormalCardLmtSum implements Function<JSONObject, Object>, RhParamKey {

    static String resultKey="secd_NormalCardLmtSum";
     ...
     * 且信贷交易信息明细（CreditDetail ）—》最新表现信息段（Crt2_P_TheLatestPerformance）—》账户状态 （AccountState）=1-正常
     * 2、取 信贷交易信息明细（CreditDetail ）—》基本信息段（Crt2_P_EssentialInformation）—》账户授信额度（CreditLineOfAccount）汇总
     *
     *
     * 1、当信贷交易信息明细（CreditDetail ）—》基本信息段（Crt2_P_EssentialInformation） —》账户类型（AccountType）=R2-贷记卡账户 且币种（Currency） 为CNY人民币
     * 且信贷交易信息明细（CreditDetail ）—》最新表现信息段（Crt2_P_TheLatestPerformance）—》账户状态 （AccountState）=1-正常
     * 或 最近一次月度表现信息（Crt2_P_LastMonthly）—》账户状态 （AccountState）=1-正常
     * 2、取 信贷交易信息明细（CreditDetail ）—》基本信息段（Crt2_P_EssentialInformation）—》账户授信额度（CreditLineOfAccount）汇总
     * @param jsonObject
     * @return
     */
    ...
    public Object apply(JSONObject jsonObject) {
        ...
        if (account.isEmpty()||StringUtils.isEmpty(jsonObject.getString("crt2_p_essentialinformation"))) {
            return amount.toString();
        }
